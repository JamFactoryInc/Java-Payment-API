...
public class UserSqlDAO implements UserDAO {
    ...
    public String findUsernameByAccountId(int id) {
        return jdbcTemplate.queryForObject("select u.username from users as u INNER JOIN accounts as a ON a.user_id = u.user_id where a.account_id = ?", String.class, id);
    }
    ...
    @Override
    public List<BasicUser> findAllUnauthenticated() {
        List<BasicUser> users = new ArrayList<>();
        String sql = "select username, user_id from users";

        SqlRowSet results = jdbcTemplate.queryForRowSet(sql);
        while(results.next()) {
            BasicUser user = new BasicUser(results.getString(1), results.getInt(2));
            users.add(user);
        }
